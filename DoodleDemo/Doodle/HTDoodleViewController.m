...
@interface HTDoodleViewController ()<UIImagePickerControllerDelegate,UINavigationControllerDelegate,HTHSVPickerViewDelegate,UIActionSheetDelegate,HTImageCroperDelegate,HTDoodleDrawViewDelegate>
{
...
@implementation HTDoodleViewController
...
- (void)setupUI {
	...
	[photoButton setImage:[UIImage imageNamed:@"doodle_photo"] forState:UIControlStateNormal];
	photoButton.frame = CGRectMake(0.0f, 0.0f, 44.0f, 44.0f);
	[photoButton addTarget:self action:@selector(photoClick) forControlEvents:UIControlEventTouchUpInside];
	self.navigationItem.titleView = photoButton;
...
//	}
	...
	NSArray *itemImages = @[@"doodle_pan", @"doodle_eraser", @"doodle_transparency", @"doodle_undo", @"doodle_redo", @"doodle_del"];
	for (int i = 0; i < itemImages.count; i ++) {
		UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
		button.tag = i;
		[button setImage:[UIImage imageNamed:[itemImages objectAtIndex:i]] forState:UIControlStateNormal];
		if (i == 2) {
			button.frame = CGRectMake((toolBar.width - itemImages.count * 52.0f) / (itemImages.count + 1) * (i + 1) + i * 52.0f + 14.0f, 1.0f + 13.0f, 24.0f, 24.0f);
			...
			button.frame = CGRectMake((toolBar.width - itemImages.count * 52.0f) / (itemImages.count + 1) * (i + 1) + i * 52.0f, 1.0f, 52.0f, 50.0f);
	...
	UIImageView *triangleImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"doodle_pop_up_triangle"]];
	CGRect frame = triangleImageView.frame;
	...
	frame = penSliderPanelView.frame;
	...
	[penSliderPanelView addSubview:triangleImageView];
	...
	triangleImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"doodle_pop_up_triangle"]];
	frame = triangleImageView.frame;
	...
	[eraseSliderPanelView addSubview:triangleImageView];
    ...
    [colorPanelView addSubview:triangleImageView];
